...
import {
  REMOTE_SOFTWARE_DETECTED,
  SEND_SOFTWARE_LIST,
} from "./types/remotecontrolTypes";
...
export const useElectronActions = (dispatch) => {
  ...
  const sendElectron = ({ type, payload }) => {
    window.api.request(type, payload);
  ...
  const getElectronVar = async (varName) => {
    ...
    return await window.electron.remote_pre(varName);
  };
  ...
  const onLogoutReply = async () => {
    window.api.response(LOGOUT_REPLY, async (event, state) => {
  ...
  const onRemoteSoftwareReply = () => {
    window.api.response(ELECTRON_REMOTE_SOFTWARE_ACTIVATED, (event, state) => {
      // if state is true, a new display was added, is false, all external display was removed
      if (state) {
  ...
  const onProcessesAqcuired = () => {
    window.api.response(ELECTRON_SEND_SOFTWARE_LIST, (event, state) => {
      // if state is true, a new display was added, is false, all external display was removed
      if (state) {
        dispatch({ type: SEND_SOFTWARE_LIST });
        window.api.request(SEND_SOFTWARE_LIST_TO_MW);
      } else {
        dispatch({ type: SEND_SOFTWARE_LIST });
      }
    });
  ...
  const onShowBiometricWindowReply = () => {
    window.api.response(SHOW_BIOMETRIC_WINDOW_REPLY, (event, state) => {
  ...
  const onBiometricPictureTakedReply = () => {
    window.api.response(BIOMETRIC_PICTURE_TAKED_REPLY, (event, arg) => {
  ...
  const onCanStartQuizReply = () => {
    window.api.response(CAN_START_QUIZ_REPLY, (event, arg) => {
      dispatch({ type: AUTH_CAN_START_QUIZ });
    });
  };

  const onQuizWindowOpnened = () => {
    window.api.response(QUIZ_WINDOW_IS_OPENED_REPLY, (event, arg) => {
   ...
   */
  const onAskState = () => {
    window.api.response(ASK_STATE, (event, arg) => {
      sendElectron({ type: ASK_STATE_REPLY, payload: auth });
    });
  };

  const onRequestStateReply = (state) => {
    window.api.response(REQUEST_STATE_REPLY, (event, state) => {
  ...
  const onConfirmWindowOpen = (state) => {
    window.api.response(CONFIRM_WINDOW_SET_STATE, (event, state) => {
      dispatch({ type: CONFIRM_WINDOW_INIT_STATE, payload: state });
    });
  };

  const onNoActiveCohort = () => {
    window.api.response(NOTIFY_NO_ACTIVE_COHORT_REPLY, (event, state) => {
      dispatch({ type: AUTH_SET_ACTIVE_COHORT, payload: null });
    });
  };
  const onConfirmWindowClosed = () => {
    window.api.response("CONFIRM_WINDOW_CLOSED_REPLY", (event, state) => {
