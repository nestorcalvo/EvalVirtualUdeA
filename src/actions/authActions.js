...
export const useAuthActions = (dispatch) => {
  const { sendElectron, getElectronVar, onLogoutReply, onAskState } =
    useElectronActions(dispatch);
  const {
    state: { auth },
  } = useStore();
  ...
  const getCohorts = async () => {
    ...
    try {
      // const headerAuth = await getTokenAuth()
      const cohorts = await httpClient
        .get("cohorts/people")
  ...
  const loginCohort = async (cohortId) => {
    ...
    try {
      const response = await httpClient.post(
        `inscriptions/${cohortId}/sessions`,
        body
      );
      if (response) {
  ...
  const setActiveCohort = async (cohortId, session) => {
    ...
    const cohort =
      auth.cohorts[auth.cohorts.findIndex((c) => c.id === cohortId)];
  ...
  const loadDeviceInfo = async () => {
    try {
      ...
      return {
        deviceInfo,
        mac: serverHost[0]?.mac,
        localIp: serverHost[0].address,
      };
    } catch (error) {
  ...
  const getUserInscriptions = async () => {
    ...
    try {
      const inscriptions = await httpClient
        .get("inscriptions")
        .then((response) => response.json());
